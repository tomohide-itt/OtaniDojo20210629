...
namespace arr{
	...
	template<typename T>
	matrix<T>& matrix<T>::replace( const matrix<T>& m ){
		unsigned int min_rows = ( mrows < m.rows() )? mrows : m.rows();
		unsigned int min_cols = ( mcols < m.cols() )? mcols : m.cols();
	...
	matrix<T>& matrix<T>::replace( const array2<T,Nr,Nc>& m ){
